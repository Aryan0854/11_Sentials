...
class AnomalyDetection:
    def __init__(self):
        self.model = IsolationForest(n_estimators=100, contamination=0.1)  # Create model with specified parameters
        self.data: List[List[int]] = []

    def train_model(self, data: List[List[int]]) -> None:
        self.model.fit(data)  # Train the Isolation Forest model
